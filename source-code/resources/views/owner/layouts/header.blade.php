    ...
    <div class="navbar-header">
        ...
        @if (isAddonInstalled('PROTYSAAS') > 1)
            @if (!ownerCurrentPackage(auth()->id()))
                <div class="d-flex">
                    <div class="p-1 radius-10 text-center" style="background-color: #ffc107;color: #74673f;">
                        {{ __('Currently you doesn\'t have any subscription!') }} <a
                            href="{{ route('owner.subscription.index', ['current_plan' => 'no']) }}"
                            class="btn btn-danger px-1" title="{{ __('Choose a plan') }}">{{ __('Choose a plan') }}</a>
                        <button type="button" class="close topBannerClose"><span
